...
I'm not clear exactly why we're using module syntax here. Maybe if I read
[this entire document](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules)
I'd get it?

### What happens when I use `type="module`?

Modules cannot be run on localhost, their loading is automatically deferred (as
in `defer` attribute for loading), and only executed once (even if called
multiple times)

Modules are used for splitting JavaScript up into smaller programs. In this
specific case (writing "Hello World") but I understand React apps can get big!
To that end, I expect Kent C. Dodds is preparing me for that. I also know from
my previous experience with React that we make lots and lots of modules; every
component I've worked with has been a module.

### What's the difference between `append` and `appendChild`?

appendChild is way older and only supports Nodes (and only one at a time), while
append supports appending text or Nodes (even multiple at once)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=ianjmacintosh%40gmail.comianjmacintosh%40gmail.comianjmacintosh%40gmail.com).
